...
const Cards = () => {
  ...
  const getAds = async () => {
    try {
      const res = await fetch("/home/card/popular", {
        method: "GET",
      });
  ...
  function GetCard(props) {
    return (
        <Link to={props.route} className="card card2">
            <img src={`/uploads/${props.imgAd}`} className="card-img-top" alt="Imagem do Anúncio"/>
  ...
  return (
    <div className="row">
        ...
        data && data.slice(0, 8).map((item) => (
          <SplideSlide>
            <GetCard route={`/anuncio/${item.id}`} title={item.titulo} price={item.preco} imgAd={item.imagem} />
          </SplideSlide>
        ))
        }
